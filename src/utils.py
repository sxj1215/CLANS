...
def get_model(model,str='str'):
    ...
    torch.save(model, '\model_{}.pkl'.format(str))
    return '\model_{}.pkl'.format(str)


def set_model_(model, path):
    '''model.load_state_dict(deepcopy(path))
    return'''
...
########################################################################################################################
def fisher_matrix_diag_(t, x, y, model, criterion, sbatch=20):
    # Init
    fisher = {}
